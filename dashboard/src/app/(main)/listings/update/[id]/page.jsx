...
// All the icons and local data imported from the Create Page
...
const UpdatePage = () => {
      ...
      try {
        ...
        if (response.success) {
          ...
          const existingPreviews = listing.media.map((m, index) => ({
            src: m.url,
            tempId: `new-${Date.now()}-${Math.random().toString(36).slice(2, 11)}`,
        ...
        setToast({ type: "error", message: "Failed to load listing data." });
      } finally {
  ...
  const [owner, setOwner] = useState("xyz owner") // Should be from auth context
  const [agent, setagent] = useState("xyz agent") // Should be from auth context
  ...
  const handleFileChange = async (e) => {
    ...
    for (const file of selectedFiles) {
      ...
      const tempId = `new-${Date.now()}-${Math.random().toString(36).slice(2, 11)}`;
  ...
  // =========================================================================
  // 💡 Overriding Media Handling for Update Logic
  // =========================================================================

  // Adapted removeFile to track files marked for deletion
  const removeFile = (index) => {
    const fileToRemove = previews[index];

    if (fileToRemove.isExisting) {
      // Mark existing file for deletion
      setRemovedMediaIds(prev => [...prev, fileToRemove.public_id]);
    } else {
      // Remove new file from the upload queue
      const newFileIndex = files.findIndex(f => f.name === fileToRemove.name);
      if (newFileIndex !== -1) {
    ...
    // Standard array filtering for previews and order adjustment
    setPreviews(prev => prev.filter((_, i) => i !== index));
    setMediaOrder(prev => prev.filter((_, i) => i !== index));

    // Adjust cover photo index logic (same as Page.jsx)
    // ...
  };

  // Adapted reorderFiles to update the mediaOrder array
  const reorderFiles = (startIndex, endIndex) => {
    ...
    const [movedId] = newMediaOrder.splice(startIndex, 1);
    newMediaOrder.splice(endIndex, 0, movedId);
  ...
  const FileCard = ({ preview, index, onRemove, onReorder, onSetCover, isCover, canSetCover }) => {
    ...
    return (
      ...
      >
        ...
        {/* Position Indicator */}
        <div className="absolute top-2 right-2 bg-black bg-opacity-70 text-white px-2 py-1 text-xs z-10">
          #{index + 1}
        </div>
        ...
        <div className="p-3">
          ...
          {/* Quick Actions */}
          <div className="flex flex-wrap gap-[14px] justify-between items-center mt-2 pt-2 border-t border-gray-200">
            <button
              onClick={() => onRemove(index)}
              className="text-red-500 cursor-pointer hover:text-red-700 transition-colors text-xs flex items-center gap-1"
            >
              <FaTrash />
              Remove
            </button>
            ...
            {canSetCover && !isCover && (
              <button
                onClick={() => onSetCover(index)}
                className="text-blue-500 cursor-pointer hover:text-blue-700 transition-colors text-xs flex items-center gap-1"
              >
                <FaStar />
                Set Cover
              </button>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="p-3 rounded-full bg-blue-100">
                      {(() => {
                        const selected = propertyTypes[activeTab]?.find(
                          (p) => p.id === selectedProperty
                        );
                    ...
                    <div>
                      ...
                      <p className="text-gray-600">
                        {propertyTypes[activeTab]?.find(p => p.id === selectedProperty)?.label || 'Unknown'}
                      </p>
                    </div>
                  </div>
                  <button
              ...
              <div className="mb-8">
                ...
                <div className="grid preview_media_list  grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {previews.map((preview, index) => (
                    <div key={preview.tempId} className="relative">
                      <FileCard
                        preview={preview}
                        index={index}
                        onRemove={removeFile}
