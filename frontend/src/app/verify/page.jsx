...
const [resendLoading, setResendLoading] = useState(false);
  ...
  useEffect(() => {
    ...
    const storedUserId = sessionStorage.getItem("unverfIedUserId");
  ...
  const handleOtpVerify = async (e) => {
    e.preventDefault();
    if (!userId) {
      setToast({ type: "error", message: "User ID not found. Please restart verification." });
      return;
    }
    ...
    try {
      ...
      const res = await fetch("http://localhost:3001/api/verifyEmail", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ userId, otp: combinedOtp }),
      });
