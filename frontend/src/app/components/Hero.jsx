...
const handleSearch = () => {
  ...
  // 2️⃣ Remove empty filters
  const validFilters = Object.entries(filters).filter(([_, value]) => value && value.trim() !== "");

  // 3️⃣ Create query string
  const queryString = new URLSearchParams(Object.fromEntries(validFilters)).toString();

  // 4️⃣ Push user to results page
