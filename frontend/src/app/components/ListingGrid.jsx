...
const ListingCard = ({ listing }) => {
  // Helper function to get property type label and styling
  ...
  // Helper function to get listing type styling
    ...
    <div className="card group rounded-xl overflow-hidden bg-white shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-200 hover:border-blue-300">
      <Link href={`/properties/${listing._id}`}>
        <div className="img_box relative h-48 sm:h-52 md:h-56 overflow-hidden">
          ...
          {/* Listing Type Badge */}
          <span className={`absolute flex items-center gap-2 right-4 top-4 text-xs font-semibold text-white px-3 py-2 rounded-[50px] backdrop-blur-sm border border-white/30 transition-all duration-200 hover:backdrop-blur-md ${getListingTypeStyle(listing.listingType)}`}>
        ...
        <div className="content bg-white p-5">
          {/* Property Type */}
          <span className={`text-sm font-bold ${propertyTypeInfo.color} uppercase tracking-wide`}>
...
const Pagination = ({ pagination, searchParams }) => {
  const generatePageUrl = (page) => {
    // ✅ Safely convert searchParams to URLSearchParams
    ...
    <div className="flex items-center justify-between border-t border-gray-200 pt-6">
      ...
      <div className="flex items-center space-x-2">
        <Link
          ...
          className={`relative inline-flex items-center px-3 py-2 text-sm font-medium rounded-md border border-gray-300 ${
